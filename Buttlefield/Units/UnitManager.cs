...
public class UnitManager : MonoBehaviour
{
    ...
    {
        var units = GridRegistry.Instance.GetAllUnitsWithType(PlacementType.Battlefield);
    ...
    public void OnUnitOutOfCombat(UnitLogic unitLogic)
    {
        var owner = unitLogic.UnitStatsLogic.Ownership;
        var isAlive = IsUnitsAlive(owner);
        if (isAlive) return;
        ClearUnits();
        OnLooser?.Invoke(owner);
        NextState();
    }
