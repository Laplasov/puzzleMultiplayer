...
class GridBuilder : MonoBehaviour
{
    ...
    public GridBuilder PopulateGridMarks()
    {
        ...
        Quaternion prefabRotation = _cellMark.transform.rotation;
        ...
        for (int x = 0; x < _config.dimensions.x; x++)
        {
            for (int y = 0; y < _config.dimensions.y; y++)
            {
                ...
                GridMarks[position] =
                    Instantiate(_cellMark, worldPosition, prefabRotation, MarksParent.transform)
                    .AddComponent<SpaceMark>();

                GridMarks[position].transform.localScale = new Vector3(_config.sizeForPrefab.x, GridMarks[position].transform.localScale.y, _config.sizeForPrefab.y);
